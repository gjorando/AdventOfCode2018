    ...
    def __init__(self, init_list):
        ...
        for coord in np.transpose(np.where(np.multiply(self._map != "#", self._map != "."))):
            ...
            if player == self.goblin_identifier:
                ...
                self._elves.append(Fighter(pos, str(player)))
            else:
...
def play_file(path, verbose=False):
    ...
    score = sum([f._health for f in battle_map._elves+battle_map._goblins])
    result = battle_map._rounds_count * score
    return result

print("Answer for puzzle #29: {}".format(play_file("input", verbose=True)))
